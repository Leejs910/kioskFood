...
function card() {
    ...
    for (i = 0; i < foodData.length; i++) {
        if (where.text == foodData[i].title) {
            ...
            images.src = `img_2/${chosenImage}.jpg`
...
function printOrNot(){
    ...
    function print1(){
        ...
        function printPage(){
            ...
            `
            <div class="grid justify-content-center align-items-center py-1" 
            style="-bs-rows: 1; --bs-columns: 2; --bs-gap: 0rem; font-size:3px; border-top: 2px solid black; border-bottom: 2px solid black;">
                    <div>합계금액</div>
                    <div>${allPrice.textContent}</div>
                </div>
                <div class="pt-2 mb-2">주문번호 : <span>${orderNum}</span></div>
                `
